...
public class CertificateSignedRequestHandlerTest {
    ...
    void verifyExpiredCertificateIsRejected() {
        requestHandler.handle(DEFAULT_MESSAGE_ID, new CertificateSignedRequest().withCertificateChain(stringToCiStringsList(expiredCertificate)));

        verify(stationMessageSenderMock).sendCallResult(any(), messageCaptor.capture());
        assertThat(messageCaptor.getValue().getStatus().value()).isEqualTo(CertificateSignedStatusEnum.REJECTED.value());
    ...
    void verifyEmptyCertificateIsRejected() {
        requestHandler.handle(DEFAULT_MESSAGE_ID, new CertificateSignedRequest().withCertificateChain(stringToCiStringsList("NotACert")));
    ...
    void verifyCertificateWithInvalidSerialIsRejected() {
        requestHandler.handle(DEFAULT_MESSAGE_ID, new CertificateSignedRequest().withCertificateChain(stringToCiStringsList(wrongSerialCertificate)));
    ...
    void verifyV2GCertificateTypeIsRejected() {
        requestHandler.handle(DEFAULT_MESSAGE_ID, new CertificateSignedRequest()
                .withCertificateType(CertificateSigningUseEnum.V_2_G_CERTIFICATE)
                .withCertificateChain(stringToCiStringsList(wrongSerialCertificate)));
    ...
    void verifyValidCertificateTypeIsCorrectlySet() {
        requestHandler.handle(DEFAULT_MESSAGE_ID, new CertificateSignedRequest().withCertificateChain(stringToCiStringsList(validCertificate)));

        verify(stationMessageSenderMock).sendCallResult(any(), messageCaptor.capture());
        assertThat(messageCaptor.getValue().getStatus().value()).isEqualTo(CertificateSignedStatusEnum.ACCEPTED.value());
    ...
    private CiString.CiString10000 stringToCiStringsList(String certificate) {
        return new CiString.CiString10000(certificate);
    }
