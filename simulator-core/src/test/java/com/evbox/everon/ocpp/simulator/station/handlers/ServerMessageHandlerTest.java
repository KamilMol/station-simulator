...
public class ServerMessageHandlerTest {
    ...
    @BeforeEach
    void setUp() {
        serverMessageHandler = new ServerMessageHandler(stationMock, stationStoreMock, stationMessageSenderMock, null, STATION_ID, subscriptionRegistryMock);
    ...
    void verifyCallMessageWithResetPayload() throws JsonProcessingException {
        ...
        ResetRequest payload = createResetRequest().withType(ResetEnum.IMMEDIATE).build();
        ...
        assertAll(
                ...
                () -> assertThat(requestCaptor.getValue().getType()).isEqualTo(ResetEnum.IMMEDIATE)
        );
    ...
    void verifyCallMessageWithChangeAvailabilityPayload() throws JsonProcessingException {
        ...
        ChangeAvailabilityRequest payload = new ChangeAvailabilityRequest()
    ...
    void verifyCallCallError() {
        ...
        String callJson = createCall()
                .withMessageId(DEFAULT_MESSAGE_ID)
                .withAction("UnknownAction")
                .withPayload(payload)
                .toJson();

        assertThrows(UnknownActionException.class, () -> serverMessageHandler.handle(callJson));

        verify(stationMessageSenderMock).sendCallError(eq(DEFAULT_MESSAGE_ID), eq(CallError.Code.NOT_IMPLEMENTED), any());
    }
}
