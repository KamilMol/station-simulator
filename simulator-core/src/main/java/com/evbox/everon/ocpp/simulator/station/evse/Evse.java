...
public class Evse {
    ...
    public Integer startCharging() {
        ...
        charging = true;
        chargingStartAtMeter = totalConsumedWattHours;
    ...
    public long getWattConsumedLastSession() {
        return Math.max(0, totalConsumedWattHours - chargingStartAtMeter);
    }
    ...
    public Optional<Connector> tryFindPluggedConnector() {
        return connectors.stream()
                .filter(connector -> ConnectorStatusEnum.OCCUPIED.equals(connector.getConnectorStatus()))
                .findAny();
    }
